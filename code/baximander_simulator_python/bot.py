...
def frames_in_head(alpha, beta, hind, length_constants):
    try: 
        n = alpha.shape[0] + 1
    except:
        n = 2
    # load data - what are these values?
    (l_a, l_b, l_c, l_d, l_e) = length_constants
    g = []
    g.append(np.eye(3))
...
# forward matrix
def F(L):
    ans = np.array([[1, 0, L], [0, 1, 0], [0, 0, 1]], dtype=np.float)
    return ans
...
def G(a, x, y):
    ans = np.array([[math.cos(a), -math.sin(a), x],
         [math.sin(a), math.cos(a), y], [0.0, 0.0, 1.0]])
    return ans
...
# leg_act: [1 1 1 1]
def get_config(g_leg,beta,alpha,xi,leg_act,length_constants,colorSpace = 1):
    # load data
    (l_a, l_b, l_c, l_d, l_e) = length_constants
    ...
    elif leg_act[0] == 1 and leg_act[3] == 1:
        flag = 1
    ...
    xi = func(xi_pre, l_a, l_b, l_c, l_d, l_e, alpha, beta, FR, FL, HL, HR, flag)
...
def cond(g_leg, xi,l_a, l_b, l_c, air_leg_length, alpha, beta, flag):
    ...
    if flag == 0:
        xi = np.append(xi[0:4], [xi[3], xi[3], xi[3]])

    elif flag == 1:
        g_leg[0] = matrix_multiply(G(xi[0], xi[1], xi[2]), R(math.pi/2), F(l_c),
                                R(-math.pi/2), R(beta[0]), F(xi[3]))
...
def func(xi_pre, l_a, l_b, l_c, l_d, l_e, alpha, beta, FR, FL, HL, HR, flag):
    ...
    res1 = fmin_slsqp(target_func, xi_pre[0:4], bounds = bnds, acc=1e-08, iprint=-1)
...
def func1(x, l_c, beta, FR, FL, flag):
    temp = G(x[0], x[1], x[2])
    ans1 = matrix_multiply(temp, R(math.pi/2), F(l_c), R(-math.pi/2),
                            R(beta[0]), F(x[3])) - FR
    ans2 = matrix_multiply(temp, R(-math.pi/2), F(l_c), R(math.pi/2),
                            R(beta[1]), F(x[3])) - FL

    if flag == 0:
        ans = np.abs(ans1) + np.abs(ans2)
    elif flag == 1:
        ans = ans1
    else:
        ans = ans2

    ans = ans*np.array([[0, 0, 1], [0, 0, 1],[0, 0, 0]], dtype = np.float)
    # print("func1: ", ans)
    return ans

def func2(x, l_a, alpha, l_b, l_c, beta, HL, HR, flag):
    temp = G(x[0], x[1], x[2])
    ans1 = matrix_multiply(temp, F(l_a), R(alpha[0]), F(l_b), R(alpha[1]),
                            F(l_a), R(-math.pi/2), F(l_c), R(math.pi/2),
                            R(beta[3]), F(x[3])) - HL
    ans2 =  matrix_multiply(temp, F(l_a), R(alpha[0]), F(l_b), R(alpha[1]),
                            F(l_a), R(math.pi/2), F(l_c), R(-math.pi/2),
                            R(beta[2]), F(x[3])) - HR
