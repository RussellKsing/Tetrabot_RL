...
def computeBodyVelocity(alpha, d_alpha, beta, d_beta, hind, L, Lleg, activation, K, F):
    ...
    * parameters are all numpy arrays
    ...
    omega1 = np.dot(activation[0] * np.dot(np.transpose((myAdjoint(np.linalg.inv(g[0])))), K), myAdjoint(np.linalg.inv(g[0])))
...
def omega1Helper(activation, g, K):
    ...
    return np.dot(activation * np.dot(np.transpose((myAdjoint(np.linalg.inv(g)))), K), myAdjoint(np.linalg.inv(g)))
...
def framesInHead(alpha, beta, hind, L, Lleg):
    ...
    for i in xrange(1, n+1):
        ...
        # pprint (alpha[i-1])
        # pprint (np.dot(g[i-1], F))
        # pprint (np.dot(np.dot(g[i-1], F), rotationFrame(alpha[i-1])))
        # pprint (np.dot(np.dot(np.dot(g[i-1], F), rotationFrame(alpha[i-1])), F))
        # sys.exit()
    ...
    # pprint (alpha[n-1])
    # pprint (g[n-2])
    # pprint (np.dot(g[n-2], F))
    # pprint (np.dot(np.dot(g[n-2], F), rotationFrame(alpha[n-1])))
    # pprint (np.dot(np.dot(np.dot(g[n-2], F), rotationFrame(alpha[n-1])), F))
    # sys.exit()
    ...
    # pprint (beta)
    # pprint (g)
    # pprint (np.dot(g[0], np.linalg.inv(F)))
    # pprint (np.dot(np.dot(g[0], np.linalg.inv(F)), rotationFrame(beta[0])))
    # pprint (np.dot(np.dot(np.dot(g[0], np.linalg.inv(F)), rotationFrame(beta[0])), np.array([[1,0,Lleg],[0,1,0],[0,0,1]])))
    # sys.exit()
